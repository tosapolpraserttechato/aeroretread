...
export default function Dashboard() {
  ...
  const bottlenecks = React.useMemo(() => {
    return processInventory.filter(p => {
      const total = p.C + p.R + p.I + p.T + p.J + p.H;
      const limit = WIP_LIMITS[p.name] || 999;
      return total > limit;
    });
  }, [processInventory]);
  ...
  return (
    <div className="p-6 bg-slate-950 min-h-screen font-sans text-slate-100">
      ...
      {bottlenecks.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4 mb-8 flex items-start gap-3 animate-pulse">
          <ShieldAlert className="text-red-400 shrink-0 mt-0.5" size={20} />
          <div>
            <h4 className="text-sm font-bold text-red-200">Active Production Bottlenecks Detected</h4>
            <p className="text-xs text-red-300 mt-1">
              The following processes have exceeded their target WIP capacity limit:{' '}
              <span className="font-bold">
                {bottlenecks.map(b => `${b.fullName} (${b.C + b.R + b.I + b.T + b.J + b.H}/${WIP_LIMITS[b.name]})`).join(', ')}
              </span>.
              Recommended action: Shift personnel to help resolve these overloaded stations, and regulate entry gate releases.
            </p>
          </div>
        </div>
      )}
